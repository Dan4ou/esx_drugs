		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.HeroinProcessing.coords, true) < 5 then
			...
			if IsControlJustReleased(0, Keys['E']) and not isProcessing then
				...
				if Config.LicenseEnable then
					...
					exports['mythic_notify']:SendAlert('inform', 'Ти преобработваш маковото семе!', 2500)
					RequestAnimDict("mp_arresting")
					TaskPlayAnim(GetPlayerPed(-1),"mp_arresting", "a_uncuff", 3.0, 3.0, 9700, 51, 0, false, false, false)
					ProcessHeroin()
					exports['mythic_notify']:SendAlert('success', 'Ти превърна маковото семе в хероин!', 3800)
				end
...
function ProcessHeroin()
	...
	exports['mythic_notify']:SendAlert('error', 'Нямаш достатъчно материали за обработката на хероин!', 3800)
